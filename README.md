...
One deployment's numbers, as an illustration rather than a rule: across 213 turns `haiku` produced thinking on roughly 90% of them *with no effort flag at all*, while `opus` at `max` produced it on none of 15 and `sonnet` on none of 162. Those groups are not comparable — different clients asking different kinds of question — and the one properly controlled comparison, the same question with the same system prompt, is a single observation each way. Opus does emit thinking when driven directly through the CLI with that same system prompt, so none of this points at the gateway. Take it only as: **do not assume that a bigger model and a higher effort will show you more reasoning.** If seeing the thinking is what you are after, `haiku` was the dependable one here.
