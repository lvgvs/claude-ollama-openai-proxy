...
**Model düşündüğünde bile metin her zaman geri gelmiyor.** Burada ölçülen kurulumda `opus` içeriği tamamen boş thinking blokları döndürdü — Claude Code'un kendi oturum kaydı onları `{"type":"thinking","thinking":"","signature":"..."}` olarak yazıyor, yani okunabilir hiçbir şeyin iliştirilmediği bir imza — `haiku` ise binlerce karakterlik dolu düşünme döndürdü. 213 tur boyunca `haiku` turların yaklaşık %90'ında görünür düşünme üretti (üstelik **efor bayrağı hiç yokken**), `opus` `max` ile 15 turda hiç, `sonnet` 162 turda hiç üretmedi.
