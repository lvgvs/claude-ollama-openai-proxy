...
Bir kurulumun sayıları, kural değil örnek olarak: 213 tur boyunca `haiku` turların yaklaşık %90'ında düşündü — üstelik **efor bayrağı hiç yokken**; `opus` `max` ile 15 turda hiç, `sonnet` 162 turda hiç düşünmedi. Ama bu gruplar karşılaştırılabilir değil — farklı istemciler farklı türde sorular soruyordu — ve düzgün kontrol edilmiş tek karşılaştırma, aynı sistem prompt'uyla aynı soru, her iki tarafta da tek gözlem. Opus aynı sistem prompt'uyla doğrudan CLI üzerinden sürüldüğünde düşünme üretiyor, yani bunların hiçbiri ağ geçidini işaret etmiyor. Şu kadarını al: **daha büyük model ve daha yüksek efor seçmenin sana daha fazla akıl yürütme göstereceğini varsayma.** Asıl derdin düşünmeyi görmekse, burada güvenilir olan `haiku` oldu.
